...
// Create WebSocket server first to get broadcaster
let server;
...
if (config.noTls) {
  ...
  if (!fs.existsSync(certPath) || !fs.existsSync(keyPath)) {
    ...
    try {
      ...
      execSync(`openssl req -x509 -newkey rsa:2048 -keyout "${keyPath}" -out "${certPath}" -days 365 -nodes -subj "/CN=ConnectLAN"`, {
        stdio: 'pipe'
      });
    } catch (e) {
...
server.listen(config.port, '0.0.0.0', () => {
  ...
  if (ips.length > 0) {
    ...
    } catch (e) { /* qrcode-terminal not available */ }
  }
